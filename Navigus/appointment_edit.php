...
<div style="padding:10px">
  ...
  <div class="space">
    <button id="delete" id="delete">Delete Event</button>
  </div>

  <div class="space">
    <button id="delete" id="save">Edit Event</button>
  </div>
  ...
  <div class="space">
    <div>Admin:</div>
    <div>
      <select id="resource" name="resource" disabled ng-model="appointment.admin">
        <?php
        foreach ($db->query('SELECT * FROM [admins] ORDER BY [admin_name]') as $item) {
          $selected = "";
          if ($event["admin_id"] == $item["admin_id"]) {
            $selected = " selected";
          }
          echo "<option value='" . $item["admin_id"] . "'" . $selected . ">" . $item["admin_name"] . "</option>";
        }
        ?>
      </select>
    </div>
  ...
  <div>Name:</div>
  <div><input type="text" id="name" name="name" value="<?php echo $event['appointment_user_name'] ?>"/></div>
